...
namespace MO_Patches
{
    ...
    public class Listing_MO_Patches : Listing_Standard
    {
        ...
        public void CustomDropdownLabeledEnum<T>(string label, ref T selectedValue, Dictionary<T, Action> enumActions, string tooltip = null, string enumValueStarterString = "MO_Patches_Settings", float height = 0f, float labelPct = 1f, float dropdownWidthFactor = 0.75f) where T : Enum
        {
            float height2 = (height != 0f) ? height : Text.CalcHeight(label, base.ColumnWidth * labelPct);
            Rect rect = base.GetRect(height2, labelPct);
